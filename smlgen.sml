  ...
  fun main _ =
    let
      ...
      fun getActions tokenString (([token'], actions) :: xs) =
            if token' = tokenString then SOME actions
            else getActions tokenString xs
      ...
      val prettyDec =
        ...
        o
        (fn topdec =>
           Ast.Ast (Seq.singleton {topdec = topdec, semicolon = NONE}))
      ...
      in
        fun gen args (Ast.Ast topdecs : Ast.t) =
          Ast.Ast
            (Seq.map
               (fn {topdec, semicolon} =>
                  {topdec = goTopDec args topdec, semicolon = semicolon})
               topdecs)
      end
      ...
      fun parseArg (arg: string) : string list * gen =
        case String.tokens (fn ch => ch = #":") arg of
          typ :: opts :: [] =>
            ...
            , CharVector.foldl (fn (ch, acc) => addGen acc (lookupGen ch))
                emptyGen opts
            )
        | _ =>
            raise Fail
              "Invalid generator syntax: should be format type:generators"

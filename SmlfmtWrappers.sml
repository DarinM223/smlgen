...
struct
    ...
    in
      fun hash a_ s =
        case Seq.length s of
          0 => 0wx6D52A54D
        | n =>
            ArraySlice.foldl (fn (i, acc) => combine (acc, a_ i))
              (Word.fromInt n) s
      fun op== a_ (l1, l2) =
  ...
  struct
    val op== = fn (a, b) =>
      Token.== (MaybeLongToken.getToken a, MaybeLongToken.getToken b)
    val hash = Token.hash o MaybeLongToken.getToken
  end
           ...
           | ( Record {left = t2, elems = t3, delims = t4, right = t5}
            ...
            Seq.==
              (fn ( {lab = t0, colon = t1, ty = t2}
                 ...
                 Token.== (t0, t3) andalso Token.== (t1, t4)
                 andalso ty_0 (t2, t5)) (t3, t7)
            andalso Seq.== Token.== (t4, t8) andalso Token.== (t5, t9)
